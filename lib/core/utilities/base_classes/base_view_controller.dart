...
abstract class ViewController<T> extends StateController with WidgetsBindingObserver, RouteAware {
  final Reader read;
  ...
  @protected
  ChatsStateController get chatsStateCtrl => read(chatsStateCtrlPvdr);
  @protected
  List<Chat> get chats => chatsStateCtrl.chats;
  @protected
  ResearchStateController get researchStateCtrl => read(researchStateCtrlPvdr);
  @protected
  Research get research => researchStateCtrl.research;
  @protected
  GroupsStateController get groupsStateCtrl => read(groupsStateCtrlPvdr);
  @protected
  List<Group> get groups => groupsStateCtrl.groups;
  @protected
  ResearcherStateController get researcherStateCtrl => read(researcherStateCtrlPvdr);
  @protected
  Researcher get researcher => researcherStateCtrl.researcher;
  @protected
  EnrollmentsStateController get enrollmentsStateCtrl => read(enrollmentsStateCtrlPvdr);
  @protected
  List<Enrollment> get enrollments => enrollmentsStateCtrl.enrollments;

  EnrollmentStateController get enrollmentStateCtrl => read(enrollmentStateCtrlPvdr);

  Enrollment get enrollment => enrollmentStateCtrl.enrollment;

  ParticipantStateController get participantStateCtrl => read(partStateCtrlPvdr);

  Participant get participant => participantStateCtrl.participant;

  @protected
  void addObserver(WidgetsBindingObserver observer) => WidgetsBinding.instance.addObserver(observer);
  @protected
  @override
  ...
  void setPage(int value) {
    pageStateCtrl.state = pageViewModel.copyWith(currentPage: value);
  }

  StateController<PageViewModel> get pageStateCtrl => read(pageModelPvdr.notifier);
...
final pageModelPvdr = StateProvider((ref) => const PageViewModel(numberOfPages: 0));
